...
class RuleGenerationSignature(dspy.Signature):
    """
...
class NLToPLNSignature(dspy.Signature):
    """
...
class NL2PLNModule(dspy.Module):
    def __init__(self, model: str = "openai/gpt-4o"):
        ...
        self.nl2pln : dspy.Module = dspy.ChainOfThought(NLToPLNSignature)
        self.rulegen : dspy.Module = dspy.ChainOfThought(RuleGenerationSignature)
...
class RuleValidationSignature(dspy.Signature):
    """
    ...
    or encoded as:
    (: take_to_not_leave (Implication (Take $x $y) (Leave $x $y)) (STV 0.0 1.0))
    Note that unbound $variables in the conlusion are existentially quantified.
    Valid Example:
    (: red_to_color (Implication (Red $x) (And (Color $x $red) (Name $red 'Red'))) (STV 1.0 1.0))

    Also take into consideration the Truth Value (STV strength confidence)
    a rule with low strenght or conficence might not need as much scrutiny as one with (STV 1.0 1.0)
    For example:
    (: tail_white_rule (Implication (And (TailOf $animal $tail) (Color $tail $color)) (Color $animal $color)) (STV 0.2 0.5))
    while the color of the tail does not necesaryl tell us the color of the animal. Many animals have the same color on their whole body including the tail.
    So a 20% chance that the tail indicates the same color as the animal is acceptable.
    """
...
class ProofComparisonSignature(dspy.Signature):
    """
...
def difficulty_metric(gold: dspy.Example, pred: dspy.Prediction, trace=None, pred_name=None, pred_trace=None):
    ...
    compare : dspy.Module = dspy.ChainOfThought(ProofComparisonSignature)
    validate_rule : dspy.Module = dspy.ChainOfThought(RuleValidationSignature)
    ...
    for qr in pred.queries:
        ...
        for rule in qr['rules']:
            ...
            if checkImpl(rule) == 0.0:
                return dspy.Prediction(score=0.0, feedback=
                    f"""One of the pln rule:\n {rule}
                        did not follow the right syntax.
    ...
    for i, q in enumerate(gold.queries):
        ...
        if comparison.proof_matches_expected_answer < 0.7:
            feedback_details.append(dedent(f"""
            A possible representation to solve this problem could be (note this representation might be able to solve this specific problem but in general might be missing quite a few things, use only as inspiration):
            Statments:
                {gold.statements}
            Queries:
                {q['query']}
...
if __name__ == '__main__':
    ...
    puzzle_data = build_examples_from_file("data/sentences2.json")
